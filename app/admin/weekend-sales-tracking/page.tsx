...
export default function WeekendSalesTrackingPage() {
    ...
    const [editTotalProfit, setEditTotalProfit] = useState("");
    const [editWeekendBalance, setEditWeekendBalance] = useState("");
    ...
    const handleEditClick = (order: any) => {
        ...
        setEditTotalProfit(order.totalProfit?.toString() || "");
        setEditWeekendBalance(order.weekendBalance?.toString() || "");
    ...
    const handleUpdateOrder = async () => {
        ...
        setUpdating(true);
        try {
            const orderRef = doc(db, "WeekendUserOrders", selectedOrder.id);
            await updateDoc(orderRef, {
                ...
                weekendBalance: Number(editWeekendBalance)
            });
    ...
    useEffect(() => {
        ...
        // 2. Real-time Orders Listener (Targeting WeekendUserOrders)
        const qOrders = query(collection(db, "WeekendUserOrders"), orderBy("purchaseDate", "desc"));
                ...
                <header className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4 flex items-center justify-between z-30">
                    ...
                    <div className="hidden md:block">
                        <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest">
                            Manager / <span className="text-indigo-600">W-Sales Tracking</span>
                        </h2>
                    </div>
                    <div className="flex items-center gap-6">
                        <div className="hidden sm:flex flex-col items-end">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none mb-1">Total W-Sales</span>
                ...
                <div className="p-4 md:p-8 overflow-y-auto w-full max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-sm border border-slate-100 relative overflow-hidden group">
                        <div className="relative z-10">
                            <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">Weekend Sales Analytics 📈</h1>
                            <p className="text-slate-500 font-medium">Real-time tracking of all weekend products sold.</p>
                                ...
                                return (
                                    <div key={order.id} className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 hover:border-indigo-100 transition-all group relative overflow-hidden">
                                        ...
                                        <div className="flex flex-col lg:flex-row gap-8 lg:items-center">
                                            ...
                                            {/* Financials Grid */}
                                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8 lg:border-l lg:border-slate-50 lg:pl-8">
                                                ...
                                                <div className="bg-violet-50/50 p-3 sm:p-0 rounded-2xl sm:bg-transparent border border-violet-100/50 sm:border-0 flex flex-col justify-center">
                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 flex items-center gap-1">
                                                        <Zap size={10} className="text-violet-500" /> W. Balance
                                                    </p>
                                                    <p className="text-sm sm:text-base font-black text-violet-600">{Number(order.weekendBalance || 0).toLocaleString()} <span className="text-[9px] opacity-40">ETB</span></p>
                                                </div>
                                            </div>
                        ...
                        <div className="relative w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                            ...
                            <div className="bg-indigo-600 px-8 py-6 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-sm font-black text-white uppercase tracking-widest">Edit Weekend Order</h3>
                                </div>
                                <button
                            ...
                            <div className="p-8 space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5 col-span-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Total Profit</label>
                                        <input
                                            type="number"
                                            value={editTotalProfit}
                                            onChange={(e) => setEditTotalProfit(e.target.value)}
                                            className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-600/10 focus:border-indigo-500 transition-all"
                                        />
                                    </div>

                                    <div className="space-y-1.5 col-span-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-violet-500">Weekend Balance</label>
                                        <input
                                            type="number"
                                            value={editWeekendBalance}
                                            onChange={(e) => setEditWeekendBalance(e.target.value)}
                                            className="w-full h-14 bg-violet-50/50 border border-violet-100 rounded-2xl px-6 font-bold text-violet-700 outline-none focus:ring-2 focus:ring-violet-600/20 focus:border-violet-500 transition-all"
                                        />
                                    </div>
                                </div>
